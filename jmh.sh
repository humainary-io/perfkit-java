...
#   JMH_JVM_ARGS replaces the VM every runner forwards to the forked JVM. It
#   defaults to -server -Xms1g -Xmx1g -XX:+AlwaysPreTouch -XX:+UseG1GC, stated
#   rather than inherited: host ergonomics size the heap from the machine's
#   memory and pick the serial collector on a single-processor host, so two
#   machines running an unconfigured command would measure under two VMs. Set
#   it to nothing to measure under this host's own choices.
#
...
if [[ "$ACTION" == run ]]; then
  ...
  # The same VM the recorded runners measure under, stated here rather than
  # inherited. Nothing on this command line reaches a benchmark by itself: JMH
  # forks, and a fork takes only what -jvmArgs carries, so a launcher flag is
  # a flag on the process that spawns the measurement rather than on the
  # measurement. A listing forks nothing and needs none of it.
  RUN_COMMAND=( java -jar "$JMH_JAR" )

  if [[ "${JMH_ARGS[0]:-}" != "-l" ]] \
    && ! jmh_jvm_args_overridden ${JMH_ARGS[@]+"${JMH_ARGS[@]}"}; then
    RUN_JVM_ARGS="$( jmh_jvm_args )"

    if [[ -n "$RUN_JVM_ARGS" ]]; then
      RUN_COMMAND+=( -jvmArgs "$RUN_JVM_ARGS" )
    fi
  fi

  if (( ${#JMH_ARGS[@]} > 0 )); then
    RUN_COMMAND+=( "${JMH_ARGS[@]}" )
  fi

  if (( ${#JMH_PROVIDER_JVM_ARGS[@]} > 0 )); then
    RUN_COMMAND+=( "${JMH_PROVIDER_JVM_ARGS[@]}" )
  fi
