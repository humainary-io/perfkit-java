...
# Same VM as the quantitative pass above, so the attribution this run produces
# belongs to the numbers that run reported — and stood aside from on the same
# terms, so both passes answer a caller's own -jvmArgs the same way.
if [[ -n "$JFR_JVM_ARGS" ]] && ! jmh_jvm_args_overridden "$@"; then
  JFR_COMMAND+=( -jvmArgs "$JFR_JVM_ARGS" )
fi
JFR_COMMAND+=( "$@" )
