...
without cold-path comparisons; all but `ScopeOps` measure an active circuit.

### Inventory

213 benchmark methods across 33 classes, categorized into four groups:
...
1. **Core Substrates** — framework primitives across the `*Ops` classes: Bank, Basin, Cell, Conduit,
   ...
   Scan, Scope, Sink, State, Stem, Subject, and Window.
...
methods into 12 rows, resulting in 222 rows across the 213 methods.
...
The 2026-09-06 alpha route-version comparison measured checking the source version once for an
already-current immutable route, instead of checking each ancestor's version during dispatch.
With three forks, eight 1-second warmups and ten 1-second measurements per fork on Java 26.0.1:

| STEM depth | Before, ns/admission | Route version, ns/admission |
|------------|---------------------:|----------------------------:|
| 10         |       30.740 ± 0.926 |              26.318 ± 0.990 |
| 32         |       65.524 ± 2.178 |              62.415 ± 1.819 |

Errors are JMH 99.9% confidence-interval half-widths. The depth-10 gain is resolved (about 14%);
the depth-32 intervals overlap. A shorter two-fork screen resolved no difference for depth-1 STEM
or ordinary PIPE dispatch. These are steady-subscription workloads; subscription churn and cold
materialization are not measured by this comparison. The retained local evidence directory is
`results/20260906-stem-route-review/`, including both artifacts, source snapshots and raw results.
