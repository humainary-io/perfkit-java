...
**Thread-backed component lifecycle.** Circuit creation is dominated by virtual-thread startup and
platform scheduler effects that the harness cannot stabilize; such lifecycle benchmarks yield high
relative errors that do not converge with additional iterations. Qualitatively, these lifecycle
costs
are orders of magnitude above steady-state operation costs, which is why circuits are designed to be
long-lived. If a specific one is required, measure it via a dedicated decision run and publish it
with environment details rather than maintaining a noisy benchmark row.

The exclusion is about the noise, not about lifecycle as a category. `ScopeOps` is the one lifecycle
suite retained, because none of the above applies to it: a scope owns no thread, so creating and
closing one is allocation and pointer work that repeats across runs to within a nanosecond. It is
kept because closing a scope is a cascade over the scopes and resources beneath it, and that cascade
sits off the emission path — a regression in it moves no other row in this suite. A change that made
the cascade allocate per close, rather than only where there is something to descend into, showed up
on `scope_create_close_leaf` as 256 B/op against a 168 B/op baseline.

The suite focuses on the hot path, with `ScopeOps` the one deliberate exception noted above. Every
benchmark initializes fixtures at the trial or iteration level and measures steady-state costs
without cold-path comparisons. All but `ScopeOps` own an active circuit, and all but
`DivideConquerOps.sort_sequential` route the measured work through one — that row runs the same
workload as plain recursion, as the comparison its two substrate siblings are read against.

### Inventory

242 benchmark methods across 39 classes, categorized into four groups:

1. **Core Substrates** — framework primitives across the `*Ops` classes: Bank, Basin, Cell, Channel,
   Conduit, Cortex, CrossCircuit, Cyclic, DivideConquer, Fiber, Flow, Id, Ingress, Mirror, Name,
   Pin, Pipe, PipeMap, Pool, Port, Relate, RunChange, Scan, Scope, SignalMonitor, Sink, State, Stem,
   Subject, Transit, and Window.
...
value. `StemOps` is the only parameterized class (sweeping hierarchy `depth` over four values),
expanding its 3 methods into 12 rows, resulting in 251 rows across the 242 methods.
...
enum constant would repeatedly test the same underlying path. However, four specific kinds of
...
**The same operation batched and unbatched.** `BankOps.get_cached` and `get_cached_batch` run
identical work at two scales. A single cached lookup costs a few nanoseconds — the same order as
JMH's own per-invocation overhead — so the unbatched row measures the harness floor as much as the
operation, while the batched row amortizes it. Batching also changes inlining and loop optimization,
so the difference does not isolate the harness cost. Keep both rows to compare the two execution
shapes; neither alone establishes the cost in a caller's application.

**One mechanism reached two ways.** `DivideConquerOps.sort_pinned` and `sort_field` build the same
tree, index the same `int[]`, and perform the same arithmetic on it; they differ only in whether the
latch is reached through a `Pin` or through a plain field. The pair exists because a state-access
cost measured in a loop and the same cost paid inside a per-emission callback are not the same
measurement, and only a pair holding everything else constant separates them. Read the two together
or not at all — either row alone is an emission benchmark whose score is dominated by work neither
row is about.
...
### What This Suite Cannot Resolve: Transit Admission Cost

**A provider's per-step transit admission cost is below this suite's resolution.** A change to it
can move a provider's own minimal loopback by a double-digit percentage while the most sensitive
row here moves a few percent — a real regression arriving as a result that reads like noise. The
sizes are a provider's to record; the limit is the suite's, and it does not go away with more forks
or more iterations.

The gap is structural, and adding a deeper row does not close it. One was tried —
`cyclic_emit_sustained_10m`, the same topology at a hundred times the depth of
`cyclic_emit_deep_100k`, on the theory that the per-invocation fixture and the drain were hiding the
per-step cost. It resolved a known regression *less* well than the existing shallow rows and was
dropped rather than kept. Two reasons it failed, both properties of how the rows are built:

- **The shallow rows are sensing the fixture, not the cascade.** `cyclic_emit_direct` and
  `cyclic_emit_registrar` build their conduit, pool and subscription inside the measured body over
  only 10,000 operations, so a change to admission moves their fixture cost 100× more per operation
  than it moves a deep cascade's. Their larger absolute delta is that, not depth sensitivity — which
  is why ranking rows by how much they moved identifies the wrong row as the sensitive one.
- **Verification costs a stage.** Every row here terminates in `countedLimit`, which is `limit` plus
  a `peek` into a `Tally` — two fiber stages per step where a bare loopback runs one. That roughly
  doubles the per-step work the admission cost competes against, and it cannot simply be removed: a
  count added by the caller after the drain would match whether or not the cascade ran, and evidence
  that cannot fail is not evidence.

So the resolution limit is the price of [completion evidence](#completion-evidence), not an
oversight. A terminally verified row cannot be as sensitive as an unverified loop, and this suite
requires verification.

**What to do instead.** Keep a minimal unverified loopback diagnostic outside this suite, owned by
the provider alongside the code it guards, and run it for any change to admission or to a drain
loop. Trading verification for sensitivity is the right trade for a diagnostic and the wrong one
for a published row, which is why the two live apart: this suite refuses unverified rows, and that
refusal is exactly what costs it the sensitivity. **A green suite is not sufficient on that path.**
A provider's guard, its measured sensitivity against these rows, and the regressions it caught are
recorded with that provider.

`CyclicOps.cyclic_emit_after_fanout_100k` checks a different issue: whether a cycle recovers its
fast path after a wider ingress cascade on the same circuit. The two-target fan-out and cyclic seed
are separate ingress jobs, with a quiescent transit boundary between them. No extra await or
topology job is inserted, deliberately — an extra empty drain can repair stale admission state in a
provider that carries any, and mask the regression the row exists to catch. Recovery within one
transit phase requires a separate workload: `TransitOps.transit_contraction_100k`, which seeds a
cycle behind an overflowed fan-out in the same cascade. Its terminal count includes both priming
deliveries and the complete cycle; the score is normalized per cyclic delivery. Compare it with
`cyclic_emit_deep_100k` on the same provider, as well as across provider revisions. What such a
comparison found is a property of the provider it was run against, so it is recorded with that
...
### The VM a row is measured under

Every runner — `run` included — forwards `JMH_JVM_ARGS` to the forked JVM, defaulting to
`-server -Xms1g -Xmx1g -XX:+AlwaysPreTouch -XX:+UseG1GC`. Nothing on a runner's own command line
reaches a benchmark by itself: JMH forks, and a fork takes only what `-jvmArgs` carries, so a flag
on the launcher is a flag on the process that spawns the measurement rather than on the
measurement.

It is stated rather than inherited because the host's defaults are not a constant across the
machines this suite runs on. Heap sizing scales with the machine's memory, and collector selection
is a threshold: on a host with one active processor, ergonomics picks the serial collector instead
of G1. An unconfigured run on a constrained CI container would therefore measure under a different
collector from the same command on a workstation — for an allocation-heavy row, a different
experiment rather than a slower one.

`JMH_JVM_ARGS` replaces the list wholesale; a `-jvmArgs` on the command line replaces it for that
invocation, and the runner then adds none of its own, so what the metadata records is one list
rather than two concatenated. Setting `JMH_JVM_ARGS` to nothing measures under the host's own
ergonomics, which is worth seeing deliberately and not worth comparing across machines.
...
| Single round trip              | `_roundtrip`       | One admission and one `await()` per operation      | ns/round trip                              |

`PipeOps.async_emit_batch` and `async_emit_admission_batch` illustrate the first distinction, and
`IngressOps.ingress_contended_emit_batch` and `ingress_contended_admission_batch` draw the same one
under producer contention. `_admission_batch` avoids in-body awaiting and declares a
`TerminalVerification` parameter, which drains and validates after the timer stops. Because GC
profilers observe the entire iteration teardown, admission benchmark allocation remains end-to-end.
...
**Awaiting per operation is prohibited for any row that prices an operation.** Awaiting after each
individual operation measures the ~8 µs caller-to-worker thread rendezvous rather than the
nanosecond-scale operation. To measure operation cost, batch emissions against a single trailing
drain. To measure latency within a single traversal, use the API-level `Pulse` construct.

A `_roundtrip` row is the one shape that awaits per operation, because the rendezvous is its
subject rather than its overhead. `IngressOps.ingress_idle_roundtrip` exists to catch a lost wakeup
on an otherwise quiet circuit, a fault that leaves behavior correct and moves only latency, by
about three orders of magnitude. Such a row must say in its own documentation that it cannot resolve
admission, and must never be read as an operation cost.
...
- **Bimodal compilation**: A third shape, distinguishable from both only by looking at forks
  individually. Each fork is internally tight; the forks disagree, clustering around two values
  rather than scattering. That is the compiler reaching different inlining decisions, not the
  operation varying, and neither more iterations nor more forks converges it — more forks only
  estimate the mixture better. A mean and an error bar describe this badly: they report a value the
  benchmark never produces.

  A worked case: ten forks of one fan-out row, three iterations each, where eight forks beat the
  baseline and two landed about 70% slower, each fork internally tight. Six shape variants were
  measured and the same source produced both modes across runs, so the mode was not attributable to
  any one of them — the finding was the mixture, not a score. Read it as the shape to look for
  rather than as a figure: which rows do this, and at what split, is a property of a provider and
  belongs in its own record.

**Read forks individually before trusting a wide error bar.** `./jmh.sh table` reports the pooled
mean; the per-fork means are in the retained JSON under `primaryMetric.rawData`. A row whose forks
split into two clusters is a different finding from one whose forks scatter, and only the first is
visible this way.
...
(`Routing.STEM`) against subscriber-linked outlet chains. Scores are per source admission,
including the drain and all `depth` terminal deliveries; they do not isolate a per-hop cost.

`CrossCircuitOps` measures the one topology the rows above cannot: a target owned by **another**
circuit, where an emission is admitted twice and run by two workers. It pairs a plain pipe with a
conduit channel as the target, because what the owning circuit does with the item differs while this
side of the boundary does not, and reads against `PipeOps.async_emit_batch` for what the crossing
costs. Its own file explains why all four of its rows should be read together rather than
...
### Where a Result Is Recorded

**Not here.** This suite measures an API and resolves whatever provider its coordinates name, so a
score is a fact about that provider on that host, not about the suite — the same way this suite is
named for what it measures rather than for one provider's internals. A comparison between two
revisions of a provider is recorded with that provider, where the change it explains lives and
where the next person to touch that code will look.

What belongs here is what survives a change of provider: how to measure, how to normalize, what a
row can and cannot resolve, and how to read a result. Where a finding is worth keeping as a
*shape* — a bimodal row, a workload that cannot resolve a cost — state the shape and leave the
figures with the provider that produced them.
...
NameOps.name_depth_control_batch_10k   0.245  0.003  4,081,632,653
NameOps.name_depth_varied_batch_10k    0.426  0.002  2,347,417,840
PipeMapOps.baseline_plain_batch        8.227  0.359    121,550,991
PipeOps.async_emit_batch               8.246  0.630    121,270,919
PipeOps.empty_emit_batch               7.893  0.260    126,694,539
StemOps.stem_emit_batch[depth=1]       9.062  0.624    110,350,916
StemOps.stem_emit_batch[depth=3]      11.224  1.039     89,094,797
StemOps.stem_emit_batch[depth=5]      14.381  2.605     69,536,194
StemOps.stem_emit_batch[depth=10]     19.220  1.085     52,029,136
```

**These numbers illustrate the format; they are not a reference.** They come from one smoke run on
one machine — Apple M4, JDK 26.0.1,
`io.humainary.substrates:humainary-substrates-spi-alpha:3.2.0`, 2026-09-20 — at one fork and five
iterations, and they will not reproduce on other hardware or against another provider. The provider
named is the one this suite is developed against; it is not distributed here, and no provider is.
Read current figures from your own decision run, per
[Measurement Status](#measurement-status).

The whole `depth` sweep is shown, where an earlier excerpt of this table omitted `depth=5`: an
excerpt renders narrower than the run it came from, which is worth seeing once rather than
describing.
...
folded into four names. [Reading a Result](#reading-a-result) covers how to read them.
...
When both inputs sit beside the `metadata.txt` a decision run records, `compare` also checks the
system each was measured on: kernel, operating system release and build, CPU, and JVM. If any of
them differ it prints a warning naming each difference. A result across such a change cannot
attribute a difference to the code, because the system moved too. Only fields both runs recorded
are compared, so a run from before the OS release and build were recorded is checked on kernel,
CPU and JVM alone.
...
### Adding a Class to the Inventory

A new benchmark class is not finished when it runs. [Inventory](#inventory) states a method count, a
class count, a rendered-row count, and a list naming every Core Substrates class; all four go stale
silently, because nothing checks them. Update them in the same change that adds the class.

Take the counts from the source rather than from arithmetic on the numbers already written there:

```bash
grep -rcE '^\s*@Benchmark\s*$' src/main/java | grep -v ':0$' | wc -l   # classes
grep -rhE '^\s*@Benchmark\s*$' src/main/java | wc -l                  # methods
```

Matching `@Benchmark` loosely counts `@BenchmarkMode` too and overstates the method count by one per
class. Rendered rows are the method count plus the expansion of every `@Param` sweep, so a class
that adds no parameters adds equally to both.

Check the prose as well as the counts. Statements scoping the suite as a whole — that every row
measures an active circuit, say — are claims a new class can falsify without touching a number.
...
- [Substrates API](https://github.com/humainary-io/substrates-api-java) — Substrates specification
  and Java projection
- [Serventis API](https://github.com/humainary-io/serventis-api-java) — Serventis architectural
  overview and semantic ascent
