...
JVM_ARGS="$( jmh_jvm_args )"

COMMAND=(
  java -jar "$JMH_JAR" "$PATTERN"
  -wi "$WARMUP_ITERATIONS" -i "$MEASUREMENT_ITERATIONS" -f "$FORKS"
  -w "$WARMUP_TIME" -r "$MEASUREMENT_TIME"
  -rf json -rff "$RESULT_JSON"
)
# Omitted rather than passed empty when JMH_JVM_ARGS is set to nothing: an empty
# -jvmArgs is a request to launch a fork with one blank argument, not a request
# to leave the fork alone. Omitted too when the caller states the VM itself, so
# that what the metadata records is one list rather than two concatenated.
if [[ -n "$JVM_ARGS" ]] && ! jmh_jvm_args_overridden "$@"; then
  COMMAND+=( -jvmArgs "$JVM_ARGS" )
fi
