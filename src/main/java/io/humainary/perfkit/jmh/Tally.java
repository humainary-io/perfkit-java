...
///
/// The padding is convention, not a measured optimization. A decision run of the
/// `WindowOps` consumption benchmarks against an otherwise identical build
/// writing to a plain `long` on the state object resolved no difference on any
/// row, the deltas falling inside their combined error and pointing in both
/// directions. The likely reason there is nothing to protect: a measured loop
/// reads its pipe reference once and the JIT hoists it, so the loop never
/// re-reads the line the circuit context is writing near. Keep the holder for
/// uniformity across benchmark classes and because it is free, not on the belief
/// that removing it would cost anything. That run measured one provider, so its
/// figures live with that provider rather than here.
///
