...
public class TerminalVerification {
  ...
  /// A second circuit drained after [#drain], for a benchmark whose work
  /// crosses a circuit boundary: the first circuit's drain is what hands the
  /// work to the second, so completion is established only once both have run.
  private Circuit then;
  private long    least;
  private long    most;
  ...
  /// Verifies `expected` deliveries after draining `first` and then `second`
  /// outside the timer.
  ///
  /// The order is the order the work travels in: a cross-circuit emission is
  /// admitted to `second` while `first` drains, so draining them the other way
  /// round can observe `second` empty before it has been handed anything.

  public void expect (
    final Tally tally,
    final long expected,
    final Circuit first,
    final Circuit second
  ) {
