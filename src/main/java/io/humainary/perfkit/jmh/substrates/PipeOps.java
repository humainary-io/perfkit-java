...
public class PipeOps
  ...
  private              Pipe < Integer >  registeredFanOutPipe;
  ...
  ///
  /// Registered pipe fan-out - measures delivery to pipes registered on a channel.
  /// Single emission reaches 3 same-circuit pipes, each of which queues its
  /// delivery rather than being invoked during the dispatch. The companion of
  /// `async_emit_fanout_batch`, which registers receptors instead, and normalized
  /// the same way, per source admission; teardown verifies the three deliveries.
  ///

  @Benchmark
  @OperationsPerInvocation ( BATCH_SIZE )
  public void async_emit_registered_fanout_batch (
    final TerminalVerification verification
  ) {
    ...
    for (
      ...
      registeredFanOutPipe.emit ( PAYLOAD );
    }
  ...
  public void setupIteration () {
    ...
    // Registered pipe fan-out - three same-circuit pipes registered on one channel
    final var registeredConduit =
      circuit.conduit (
        Integer.class
      );

    registeredConduit.subscribe (
      circuit.subscriber (
        cortex.name ( "registered.fanout" ),
        ( subject, registrar ) -> {
          registrar.register ( circuit.pipe ( countedReceptor () ) );
          registrar.register ( circuit.pipe ( countedReceptor () ) );
          registrar.register ( circuit.pipe ( countedReceptor () ) );
        }
      )
    ...
    registeredFanOutPipe =
      registeredConduit.get ( name );
