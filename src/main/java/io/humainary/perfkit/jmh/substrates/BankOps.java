...
public class BankOps
  ...
  ///
  /// Benchmark cached conduit retrieval by name.
  ///
  /// Measures the steady-state cost of `Bank.get(Name)` for a name that
  /// has already been materialized. The provider decides whether a repeated
  /// lookup uses a cache hint or its backing map.
  ///
  ...
  ///
  /// Benchmark cached conduit retrieval by name, batched.
  ///
  /// The same operation as [#get_cached], amortizing JMH's per-invocation
  /// overhead across [#BATCH_SIZE] lookups. A single lookup costs a few
  /// nanoseconds, so the two rows resolve different things: the unbatched
  /// row carries more harness overhead. Batching also changes inlining and
  /// loop optimization, so their gap does not isolate the harness cost.
  ///

  @Benchmark
  @OperationsPerInvocation ( BATCH_SIZE )
  public void get_cached_batch (
  ...
  ///
  /// Benchmark cached conduit retrieval across varied names.
  ///
  /// Cycles through [#POOL_SIZE] pre-materialized names. Where [#get_cached]
  /// repeats one name, this row varies it within the same pre-populated
  /// working set. The pair measures sensitivity to that access pattern,
  /// not general best- and worst-case lookup bounds.
  ///
